...
impl PipelineDescriptor {
    pub fn new_render(shader_stages: ShaderStages) -> Self {
        PipelineDescriptor::Render {
            name: None,
            layout: None,
    ...
    pub fn new_compute(shader_stages: ShaderStages) -> Self {
        PipelineDescriptor::Compute {
            name: None,
            layout: None,
            shader_stages,
        }
    ...
    pub fn get_layout(&self) -> Option<&PipelineLayout> {
        match self {
            PipelineDescriptor::Render { layout, .. } => layout.as_ref(),
            PipelineDescriptor::Compute { layout, .. } => layout.as_ref(),
        }
    }

    pub fn get_layout_mut(&mut self) -> Option<&mut PipelineLayout> {
        match self {
            PipelineDescriptor::Render { layout, .. } => layout.as_mut(),
            PipelineDescriptor::Compute { layout, .. } => layout.as_mut(),
        }
    }
    ...
    pub fn reflect_layout(
        ...
        let mut layouts = vec![];
        match self {
            PipelineDescriptor::Render { shader_stages, .. } => match shader_stages {
                ShaderStages::Render { vertex, fragment } => {
                    let vertex_spirv = shaders.get(&vertex).unwrap();
                    let fragment_spirv = fragment
                        .as_ref()
                        .map(|handle| shaders.get(&handle).unwrap());

                    layouts.push(vertex_spirv.reflect_layout(bevy_conventions).unwrap());
                    if let Some(ref fragment_spirv) = fragment_spirv {
                        layouts.push(fragment_spirv.reflect_layout(bevy_conventions).unwrap());
                    }
                }
                _ => panic!("Invalid shader stage for render pipeline."),
            },
            PipelineDescriptor::Compute { shader_stages, .. } => match shader_stages {
                ShaderStages::Compute { compute } => {
                    let compute_spirv = shaders.get(&compute).unwrap();
                    layouts.push(compute_spirv.reflect_layout(false).unwrap());
                }
                _ => panic!("Invalid shader stage for compute pipeline."),
            },
        }

        let mut new_layout = PipelineLayout::from_shader_layouts(&mut layouts);
        if let Some(vertex_buffer_descriptors) = vertex_buffer_descriptors {
            new_layout.sync_vertex_buffer_descriptors(vertex_buffer_descriptors);
        }

        if !dynamic_bindings.is_empty() {
            // set binding uniforms to dynamic if render resource bindings use dynamic
            for bind_group in new_layout.bind_groups.iter_mut() {
        ...
        match self {
            PipelineDescriptor::Render { layout, .. } => {
                *layout = Some(new_layout);
            }
            PipelineDescriptor::Compute { layout, .. } => {
                *layout = Some(new_layout);
            }
        }
    }
